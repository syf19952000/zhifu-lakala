...
if (!isset($result['out_trade_no'])){
    ...
    $result = '';
    $sql = "INSERT INTO `order` (out_trade_no, transaction_id, money, xmob, nonce_str, mch_id, `result`, open_id, zt, `source`) VALUES ('$out_trade_no', '$transaction_id', $money, '$qcj', '$nonce_str', '$mch_id', '', '$open_id', 1, 'fugui')";
    ...
    if ($attach < $attach_num && $money != 45.90){
        ...
        $qcj = json_decode($qcj, true);
        $data['plid'] = $qcj['plid'];
        $data['userkey'] = $qcj['userkey'];
        $data['purchase_amount'] = $qcj['purchase_amount'];
        $data['md5phone'] = $qcj['md5phone'];
        $data['type'] = 2;

        $finalurl = "https://game.adtianshi.cn/UrlCenter/Transfer?" . http_build_query($data);
        fwrite($myfile,"Send Xmob ".$finalurl."\r\n");
        $ch = curl_init();
